...
onegreek.config(["$stateProvider", "$urlRouterProvider", "$httpProvider", function($stateProvider, $urlRouterProvider, $httpProvider) {
    ...
    $stateProvider
        ...
        .state('register.approvePhone', {
            url: '/register/approvePhone',
            templateUrl: 'views/pages/register/approvePhone.html',
            controller: 'registerController'
        })
...
onegreek.controller('registerController', ["$scope", "$state", "$timeout", "$interval", "authService", "userService", "universityService", "organizationService", "smsService", function($scope, $state, $timeout, $interval, authService, userService, universityService, organizationService, smsService) {
    ...
    $scope.clickedOnInputPhone = false;
    $scope.clickedOnInputApprovePhone = false;
    $scope.showRedQuestion = false;
    ...
    $scope.submitPhone = function() {
        if($scope.user.phone>999999999 && $scope.user.phone<10000000000) {
            $scope.showRedQuestion = false;
            smsService.sendSMS($scope.user.phone, function () {
                $state.go('register.approvePhone')});
        }
        else{
            $scope.showRedQuestion = true;
        }
    };
    $interval(function(){
        $scope.iframeHeight = window.innerHeight-100;
        if($scope.iframeHeight>300)
            $scope.bowtieMarginTop = 45;
        else
            $scope.bowtieMarginTop = 0;
    }, 1000);


    $scope.onTextClick = function () {
        $scope.clickedOnInputPhone = true;
    };

    $scope.approveCode = function() {
        smsService.checkSMS($scope.user.phone, $scope.user.phoneCode, function(res){
            //userService.save($scope.user);
            $state.go('register.email');
        });
    };

    $scope.onTextApproveClick = function () {
        $scope.clickedOnInputApprovePhone = true;
    };
...
onegreek.service('smsService', ["$http", function($http) {
    return {
        /**
         *
         * @param phone
        ...
        sendSMS: function(phone, callback) {
            ...
            //$http.get("/smsProxy/check/"+79811706016+"/"+12345).success(function (data, status) {
